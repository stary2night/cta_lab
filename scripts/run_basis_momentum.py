...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Basis momentum 中国期货回测")
    p.add_argument(
        "--data-dir",
    ...
    p.add_argument(
        "--signal-mode",
        choices=["term_structure_change", "return_spread"],
        default="return_spread",
        help="信号口径，默认 return_spread（学术基差动量）",
    )
    p.add_argument("--signal-window", type=int, default=20, help="term_structure_change 滚动窗口，默认20")
    p.add_argument("--academic-lookback", type=int, default=252, help="学术版收益差回望窗口，默认252")
    p.add_argument("--active-oi-pct-threshold", type=float, default=0.05, help="远月最小持仓量占比，默认0.05")
    p.add_argument(
        "--vol-scale-windows",
        default="20,60,120",
        help="资产/组合波动窗口，逗号分隔，默认20,60,120",
    )
...
def main() -> None:
    ...
    strategy = BasisMomentumStrategy(
        config={
            ...
            "academic_lookback": args.academic_lookback,
    ...
    out.save_parquet(result.basis_change, "signals", "basis_change.parquet")
    ...
    summary_df = pd.DataFrame([summary]).rename(index={0: "BasisMomentum"})
    ...
    out.save_fig(
        plot_nav_with_drawdown(
            {"BasisMomentum": pnl},
            title="China Futures Basis Momentum",
        ),
        "charts", "nav_basis_momentum.png", dpi=150, bbox_inches="tight",
    )
    out.save_fig(
        plot_annual_bar(
            {"BasisMomentum": annual_df},
            title="China Futures Basis Momentum — Annual Returns",
        ),
    ...
    out.save_fig(
        plot_monthly_heatmap(
            monthly_df,
            title="China Futures Basis Momentum — Monthly Returns (%)",
        ),
