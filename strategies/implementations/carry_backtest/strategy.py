"""China futures carry (roll yield) strategy.

Signal construction
-------------------
For each instrument that has a liquid far-leg contract:

1. Compute daily carry = log(near_price / far_price).
   - Positive  → backwardation (near > far): positive roll yield
   - Negative  → contango     (near < far): negative roll yield

2. Clip the raw carry to ±carry_clip (default ±0.30) to limit
   outlier influence from near-expiry price spikes.

3. Cross-sectional signal:
       signal = carry_i   (rank from most contangoed → most backwardated)

   High carry → LONG near leg (collect positive roll while curve converges).
   Low carry  → SHORT near leg (pay negative roll as curve rises against you).

Key difference from BasisValueStrategy
---------------------------------------
BasisValueStrategy z-scores the basis against its own 504-day rolling
history (within-instrument normalisation).  CarryStrategy uses the raw
log-basis directly as a cross-sectional ranking input — no lookback
window, no within-instrument standardisation.  This means:

  * The signal is available from day 1 (no warm-up beyond vol windows).
  * Cross-sectional comparison reflects structural carry differentials
    rather than deviations from each instrument's own history.
  * The strategy is purely a structural premium capture, not a timing
    signal on when carry is historically "extreme".

Portfolio construction is identical to BasisValueStrategy:
...
class CarryStrategy(StrategyBase):
    ...
    做多最高 carry（最深逆价差/backwardation）品种，
    做空最低 carry（最深正价差/contango）品种。

    信号 = log(近月价格 / 远月价格)，截断至 ±carry_clip 后截面排序。
    不对自身历史做 z-score 归一化（区别于 BasisValueStrategy）。
    """

    def __init__(
        self,
        config: CarryConfig | dict | None = None,
    ) -> None:
        ...
        self.carry_clip = cfg.carry_clip
    ...
    def generate_signals(self, price_df: pd.DataFrame) -> pd.DataFrame:
        ...
        carry = np.log(term_structure.replace([np.inf, -np.inf], np.nan))
        return carry.clip(-self.carry_clip, self.carry_clip)
    ...
    def compute_carry(
        ...
        """Build term structure ratio and carry signal.
        ...
        carry : DataFrame
            Raw log(near/far) clipped to ±carry_clip.
            Positive → backwardation → long signal.
            Negative → contango     → short signal.
        """
        ...
        carry = np.log(term_structure.replace([np.inf, -np.inf], np.nan))
        carry = carry.clip(-self.carry_clip, self.carry_clip)

        return (
            term_structure.replace([np.inf, -np.inf], np.nan),
            carry.replace([np.inf, -np.inf], np.nan),
        )
    ...
    def run_pipeline(
        ...
        """Load data, compute carry signal, and run backtest."""
        ...
        if verbose:
            ...
            print("\n" + "=" * 65)
            print("Step 2: Compute carry signal  log(near/far)")
            print("=" * 65)

        term_structure, carry = self.compute_carry(near_prices, far_prices)
        ...
        if verbose:
            ...
            backwardation_pct = (carry > 0).sum(axis=1).div(carry.notna().sum(axis=1)).mean()
            print(
                f"  Active signal share={active_share:.1%}  "
                f"Far-leg coverage={far_coverage:.1%}  "
                f"Avg backwardation pct={backwardation_pct:.1%}"
            )
            print(
            ...
            print("\n" + "=" * 65)
            print("Step 3: VectorizedBacktest — carry")
            print("=" * 65)
        ...
        if verbose and not pnl.empty:
            self._print_summary(pnl, "Carry")

        return CarryRunResult(
            returns=returns,
