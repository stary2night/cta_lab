"""Basis value (mean-reversion) CTA 中国期货回测入口。"""
...
from strategies.implementations.basis_value_backtest import BasisValueStrategy


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Basis value 中国期货回测（基差均值回归）")
    p.add_argument(
        "--data-dir",
    ...
    p.add_argument("--basis-z-lookback", type=int, default=504, help="基差z-score滚动窗口，默认504（IC分析最优）")
    p.add_argument("--z-clip", type=float, default=3.0, help="z-score截断阈值，默认3.0")
...
def main() -> None:
    ...
    strategy = BasisValueStrategy(
        config={
            "min_obs": args.min_obs,
            "basis_z_lookback": args.basis_z_lookback,
            "z_clip": args.z_clip,
    ...
    out.save_parquet(result.basis_z, "signals", "basis_z.parquet")
    ...
    summary_df = pd.DataFrame([summary]).rename(index={0: "BasisValue"})
    ...
    out.save_fig(
        plot_nav_with_drawdown(
            {"BasisValue": pnl},
            title="China Futures Basis Value (Mean Reversion)",
        ),
        "charts", "nav_basis_value.png", dpi=150, bbox_inches="tight",
    )
    out.save_fig(
        plot_annual_bar(
            {"BasisValue": annual_df},
            title="China Futures Basis Value — Annual Returns",
        ),
    ...
    out.save_fig(
        plot_monthly_heatmap(
            monthly_df,
            title="China Futures Basis Value — Monthly Returns (%)",
        ),
